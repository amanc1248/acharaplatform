...
const registerTailor = asyncHandler(async (req, res) => {
  ...
  db.query(
    finalSql,
    [first_name, last_name, phone, email, hashedPassword, city, street],
    (err, result) => {
      if (err) throw err;
  ...
  db.query(sql, email, async (err, result) => {
    ...
    if (result.length > 0) {
      if (await MatchPassword(password, result[0]["password"])) {
        const token = generateToken(result[0].tailor_id);
        res.json({
          first_name: result[0]["first_name"],
          last_name: result[0]["last_name"],
          tailor_id: result[0]["tailor_id"],
          phone: result[0]["phone"],
          email: result[0]["first_name"],
